...
The functions are very specific to the work in this Rmd file.  If they ever become desired as a more standard capability they will be moved to the functions instantiated in icicle setup.

Many of those that are disabled below have been moved to AML-Libraries and were redundant here.
...
# -------------------------------------------------------------------------------------------------------
Rows below here have been migrated to other libraries
# -------------------------------------------------------------------------------------------------------
...
Used to determine backbone and intensity
...
Classifies treatment intensity from any text column and appends a new column (either "high"/"low" labels or 1 high | 2 int | 0 low. It the get_pattern_map() function to determine treatment mapping to intensity.

This code came from work by Monica Gerber. The link to get updated patterns is:

[Categorizing Intensity and Backbone](https://github.com/FredHutch/Halpern_TDS-IRC/blob/main/Code/Cohort_Development/01_cohortid_from2298.qmd "Monica Gerber Code at https://github.com/FredHutch/Halpern_TDS-IRC/blob/main/Code/Cohort_Development/01_cohortid_from2298.qmd")

```         
Github: FredHutch / Halpern_TDS-IRC / Code / Cohort_Development / 01_cohortid_from2298.qmd
```
...
#### add_backbone(data, col, new_col, ignore_case, ties = c("first", "all"), sep, backbone_order)

Utilizes patten_map() from Monica's code [see classify_intensity()]. As part of Monica's code she first categorized patients by treatment description and then extrapolated a treatment intensity.
...
#### is_currative_rx_fhcc(treatment)
...
Removes all text coming after a pipe symbol. "\|" Makes no further changes if the text contains "/" because this is use both to mean "and" or also "or" If no "/" is found: Removes references to Salvage (like S1) or Cycle (like C1) Removes dates formatted like mm/dd/yyyy or mm-dd-yyyy or yyyy-mm-dd Removes dosage information like "daily" "injection" "Q1" "100mg/m2" "50mg/m²" "12 day" "3 days" "IV" "PO" Removes words describing the phase, location, or result like "consolidation" "maintenance" "outside" "refractory" Standardizes punctuation:\
