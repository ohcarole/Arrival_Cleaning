...
compare_col_types <- function(df1, df2, target_class = "Date") {
  t1 <- map(df1, class)
  t2 <- map(df2, class)

  common_cols <- intersect(names(t1), names(t2))

  keep <- map_lgl(common_cols, function(col) {
    c1 <- t1[[col]]
    c2 <- t2[[col]]

    (target_class %in% c1 && !(target_class %in% c2)) ||
      (target_class %in% c2 && !(target_class %in% c1))
  })
...
#   gclam      = paste0("^(?=.*", gclam_base, "(?!.*\\bmini\\b)")
